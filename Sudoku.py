...
class SudokuGame:
    ...
    def draw_menu(self):
        ...
        for i, opt in enumerate(self.menu_options):
            ...
            if is_selected:
                ...
                self.screen.blit(
                    self.font_med.render("> ", True, color),
                    self.font_med.render("> ", True, color).get_rect(
                        midright=(bg_rect.left - 10, bg_rect.centery)
                    ),
                )
                self.screen.blit(
                    self.font_med.render(" <", True, color),
                    self.font_med.render(" <", True, color).get_rect(
                        midleft=(bg_rect.right + 10, bg_rect.centery)
                    ),
                )
    ...
    def draw_game(self):
        ...
        diff_img = self.font_small.render(
            f"MODALITÀ: {self.difficulty_label}", True, (150, 150, 150)
        )
            ...
            for c in range(9):
                ...
                if val != 0:
                    ...
                    for n in self.notes[r][c]:
                        ...
                        self.screen.blit(
                            self.font_note.render(str(n), True, THEME["note"]), (nx, ny)
                        )
        ...
        if self.won or self.game_over:
            end_msg = (
                "VITTORIA! ESC per Menu" if self.won else "GAME OVER! ESC per Menu"
            )
            end_img = self.font_small.render(
                end_msg, True, THEME["win"] if self.won else THEME["error"]
            )
            self.screen.blit(
                end_img, end_img.get_rect(center=(WIDTH // 2, footer_y + 25))
            )
                ...
                if event.type == KEYDOWN:
                    if event.key == K_ESCAPE:
                        if self.state == "PLAYING":
                            ...
                            subprocess.Popen([sys.executable, "MainMenu.py"])
                            pygame.quit()
                            sys.exit()
                    if self.state == "MENU":
                        if event.key == K_UP:
                        ...
                        self.state == "PLAYING" and not self.won and not self.game_over
                    ):
                        if event.key == K_a:
